...
@router.get(
    ...
    description=(
        ...
        "화면의 메시지 여러 개의 상태가 필요하면 `?qna_uuid=A&qna_uuid=B`로 한 번에 가져온다."
    ),
)
def list_qna_logs(
    page: Pagination = Depends(),
    qna_uuid: Optional[List[UUID]] = Query(
        default=None, description="이 턴들만. 여러 번 반복해 넘긴다"
    ),
    ...
    # id 묶음 조회의 상한은 페이지 크기 상한과 같다(→ evaluations 라우터의 같은 판단)
    if qna_uuid and len(qna_uuid) > settings.max_page_size:
        raise HTTPException(
            http_status.HTTP_422_UNPROCESSABLE_CONTENT,
            detail=f"qna_uuid는 한 번에 {settings.max_page_size}개까지 넘길 수 있습니다.",
        )
    ids = [str(u) for u in qna_uuid] if qna_uuid else None

    total, rows = repo.list_logs(
        page.limit, page.offset, ids, status, answer_type, conversation_id, q
    )
    return Page(
        total=total,
        limit=page.limit,
